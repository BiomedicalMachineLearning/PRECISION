...
# Author: Laura Grice
# Date: 24th July 2020
# Title: 10c_integrationClustering.R
# Goal: To cluster one integratd sample
# ------------------------------------------------------------------
# USAGE
# Usage: Rscript 10c_integrationClustering.R {sampleID} {seuratObj} {outdir_figs} {outdir_obj} {res.override} {ndims}
# Where:
...
if (length(args) != 6) {
  ...
  cat("example: Rscript 10c_integrationClustering.R {sampleID} {seuratObj} {outdir_figs} {outdir_obj} {res.override} {ndims}\n")
  quit()
}
...
outdir_obj <- args[4]
res.override <- args[5]
#res.override <- if (grepl("\\d", res.override)) {res.override <- as.numeric(res.override)}
...
if (res.override == "no") {
  ...
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0") + ggtitle("candidate res = 0")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.1") + ggtitle("candidate res = 0.1")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.1.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.2") + ggtitle("candidate res = 0.2")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.2.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.3") + ggtitle("candidate res = 0.3")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.3.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.4") + ggtitle("candidate res = 0.4")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.4.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.5") + ggtitle("candidate res = 0.5")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.5.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.6") + ggtitle("candidate res = 0.6")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.6.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.7") + ggtitle("candidate res = 0.7")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.7.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.8") + ggtitle("candidate res = 0.8")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.8.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.0.9") + ggtitle("candidate res = 0.9")
  ggsave(paste0(outdir, sampleID, "_possClust_res0.9.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.1") + ggtitle("rcandidate res = 1")
  ggsave(paste0(outdir, sampleID, "_possClust_res1.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.1.2") + ggtitle("candidate res = 1.2")
  ggsave(paste0(outdir, sampleID, "_possClust_res1.2.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.1.4") + ggtitle("candidate res = 1.4")
  ggsave(paste0(outdir, sampleID, "_possClust_res1.4.pdf"))
  DimPlot(df.2, reduction = "umap", group.by = "integrated_snn_res.1.6") + ggtitle("candidate res = 1.6")
...
saveRDS(df, file = paste0(outdir_obj, sampleID, "integratedClustered_Seurat.RDS"))
# ------------------------------------------------------------------
# SESSION INFO
# ------------------------------------------------------------------
