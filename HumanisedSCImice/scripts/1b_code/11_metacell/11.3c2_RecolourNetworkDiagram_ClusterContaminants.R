...
# WHERE:
# seuratObject = the subsetted Seurat object used before Metacell analysis, which contains a column called "subclust"
...
r_packages <- c("Seurat", "RColorBrewer", "ggplot2", "pheatmap", "randomcoloR")
...
df <- readRDS(seurat)
clusterannotations <- df[["subclust"]]
...
DimPlot(df2, group.by = "subclust", label = TRUE, repel = TRUE) +
  NoLegend() +
  xlim(xrange) +
  ylim(yrange)
ggsave(paste0(recolour_outdir, sampleID, "_seuratplot.pdf"))
# ------------------------------------------------------------------
# SESSION INFO
# ------------------------------------------------------------------
