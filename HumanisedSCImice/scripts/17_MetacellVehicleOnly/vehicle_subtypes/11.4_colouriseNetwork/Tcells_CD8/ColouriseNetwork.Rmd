...
```{r}
sampleID <- "CD8Tcells"
df <- readRDS("/scripts/17_MetacellVehicleOnly/vehicle_subtypes/11.1a_ReplotUMAP/CD8_Tcells/out_obj/ReplotUMAP_Seurat.RDS")
outdir_metacell <- "/scripts/17_MetacellVehicleOnly/vehicle_subtypes/11.3_RunMetacell/Tcells_CD8/out_obj/"
recolour_outdir <- "/scripts/17_MetacellVehicleOnly/vehicle_subtypes/11.4_colouriseNetwork/Tcells_CD8/outdir//"
...
# there is a row with a missing name, drop this first
keep <- rownames(mc2d@sc_x[!(is.na(rownames(mc2d@sc_x)))])
scx <- as.data.frame(mc2d@sc_x)[keep,,drop = FALSE]
scy <- as.data.frame(mc2d@sc_y)[keep,,drop = FALSE]
clusterannotations2 <- clusterannotations[keep,,drop = FALSE]
networkdata <- cbind(scx, scy, clusterannotations2)
