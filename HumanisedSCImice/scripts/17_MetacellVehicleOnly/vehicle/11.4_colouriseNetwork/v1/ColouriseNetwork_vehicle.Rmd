...
outdir_metacell <- "/scripts/17_Metacell/vehicle/11.3_RunMetacell/out_obj/"
recolour_outdir <- "/scripts/17_Metacell/vehicle/11.4_colouriseNetwork/outdir/"
Idents(df) <- "integrated_snn_res.0.4"
```

```{r}
# the annotations have "-1" but the df doesn't. Remove the -1
#annotations2 <- annotations
#rownames(annotations2) <- gsub("\\-.*","",rownames(annotations2))
#df <- AddMetaData(df, annotations2)
```
...
plot_c0 <- ggplot() + 
  ...
  scale_fill_manual(values = palette_clust) +
  geom_segment(aes(x = mc2d@mc_x[fr], y = mc2d@mc_y[fr], xend = mc2d@mc_x[to], yend = mc2d@mc_y[to]), col = "gray25", size = 0.3) +
  geom_point(aes(x = mc2d@mc_x, y = mc2d@mc_y), shape = 21, fill = "white", color = "black", size = 4) +
