...
```{r}
outdir <- "/scripts/0_Figures/Fig_ReplotGORedBlue/Clusterwise/outdir/"

sampleID <- "4.1_VennOverlapSCIOnly"

ck_filt_simple <- readRDS("/scripts/14_GO/Clusterwise_AllGenes_RemoveRbMt/outdir_4.1_VennOverlaps_SCIOnly/cf_4.1_SCIOnly_top100GO_FiltSimp.RDS")
```
...
MakeDotplots <- function(n, width, height) {
  ...
  GOdata$Cluster <- gsub("SCI_only_", "", GOdata$Cluster)
  ...
  table <- mutate(table, clusterID = gsub(".*_","",Cluster))
  table <- mutate(table, direction = gsub("_.*", "", topN_allhits$Cluster))
  ...
  dotplot_grp_topN <- table %>% filter(Description %in% markers) %>% 
    ggplot(aes(x=clusterID, y = Description, color = padjdir, size = GeneRatio)) + 
    geom_point() +
  ...
  dotplot_grp_topN_withGOlabels <- table %>% filter(Description %in% markers) %>% 
    ggplot(aes(x=clusterID, y = ID_desc, color = padjdir, size = GeneRatio)) + 
    geom_point() +
    scale_size_continuous(range = c(0.5, 4)) +
    scale_color_gradientn(colours = c("blue", "lightskyblue", "white", "orange", "red"),
                          limits = c(-scaleend, scaleend)) +
...
```{r}
#n, width, height
MakeDotplots(3, 7, 7.5)
MakeDotplots(10, 7, 7.5)
MakeDotplots(20, 7, 8.5)
