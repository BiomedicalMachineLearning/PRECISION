...
```{r}
sampleID <- "4.1_VennOverlapSCIOnly"
markers_all <-read.delim("/scripts/13b_ProcessForGO/Clusterwise/outdir/4.1_SCIvsshamnaive_blood_ComparisonVenn_Markers_SCIonly.txt")
markers_noRb <- read.delim("/scripts/13b_ProcessForGO/Clusterwise/outdir/4.1_SCIvsshamnaive_blood_ComparisonVenn_Markers_RemoveRbMt_SCIonly.txt")
  
GO_noRb <- read.delim("/scripts/14_GO/Clusterwise_AllGenes_RemoveRbMt/outdir_4.1_VennOverlaps_SCIOnly/cf_4.1_SCIOnly_top100GO_FiltSimp.txt")

outdir <- "/scripts/0_Figures/Fig_NbrDEGsAndGO/Clusterwise/outdir/"
```

# Simplify the cluster IDs (remove "SCI_only_")

```{r}
GO_noRb$Cluster <- gsub("SCI_only_", "", GO_noRb$Cluster)
markers_all$cluster <- gsub("SCI_only_", "", markers_all$cluster)
markers_noRb$cluster <- gsub("SCI_only_", "", markers_noRb$cluster)
```
...
```{r}
markers_all$direction <- gsub("_.*", "", markers_all$cluster) #direction
markers_all$clusterID <- gsub(".*_", "", markers_all$cluster) #cluster

markers_noRb$direction <- gsub("_.*", "", markers_noRb$cluster) #direction
...
nbr_markers_all$clusterID <- gsub(".*_", "", nbr_markers_all$Var1) #direction
nbr_markers_all$direction <-  gsub("_.*", "", nbr_markers_all$Var1) #cluster
colnames(nbr_markers_all) <- c("cluster_direction", "# of markers", "cluster", "direction")

nbr_markers_noRb <- as.data.frame(table(markers_all$cluster))
nbr_markers_noRb$clusterID <- gsub(".*_", "", nbr_markers_noRb$Var1) #direction
nbr_markers_noRb$direction <-  gsub("_.*", "", nbr_markers_noRb$Var1) #cluster
colnames(nbr_markers_noRb) <- c("cluster_direction", "# of markers", "cluster", "direction")
```
...
colnames(nbr_markers_noRb_temp) <- c("cluster_direction", "noRb", "cluster", "direction")
nbr_markers_all_temp <- nbr_markers_all2
colnames(nbr_markers_all_temp) <- c("cluster_direction", "Rb", "cluster", "direction")
...
NbrGo$clusterID <- gsub(".*_", "", NbrGo$Var1) #direction
NbrGo$direction <-  gsub("_.*", "", NbrGo$Var1) #cluster
colnames(NbrGo) <- c("cluster_direction", "# of GO terms", "cluster", "direction")
```
