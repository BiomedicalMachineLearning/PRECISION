...
```{r}
sampleID <- "4.4+4.5"
markers_all <- read.delim("/scripts/13b_ProcessForGO/Samplewise/outdir/4.4v4.5_ComparisonVenn_Markers.txt")
markers_noRb <- read.delim("/scripts/13b_ProcessForGO/Samplewise/outdir/4.4v4.5_ComparisonVenn_Markers_RemoveRbMt.txt")
GO_noRb <- read.delim("/scripts/14_GO/Samplewise_AllGenes_RemoveRbMt/outdir_4.4vs4.5/cf_4.4_4.5_top100GO_FiltSimp.txt")
...
ggsave(filename = paste0(outdir, sampleID, "_NumberOfDEGs_all.pdf"), width = 2, height = 4)
```
...
ggplot(nbr_markers_noRb, aes(x=direction, y=`# of markers`, fill = direction)) + 
    ...
    theme_bw() + 
    geom_text(aes(label = `# of markers`), vjust = -1, color="black", size = 2)
ggsave(filename = paste0(outdir, sampleID, "_NumberOfDEGs_dropRbMt.pdf"), width = 2, height = 4)
```
...
ggplot(nbr_markers_both, aes(fill = myfill, y = value, x = direction)) + 
  ...
  scale_fill_manual(values = palette_dirRb) +
    theme(legend.position="none") +
    theme_bw()
ggsave(filename = paste0(outdir, sampleID, "_NumberOfDEGs_Rb+NoRb.pdf"), width = 3, height = 4)
```
...
ggsave(filename = paste0(outdir, sampleID, "_NumberOfGO_dropRbMt.pdf"), width = 3, height = 4)
```