...
```{r}
seurat_markers.tab <- read.delim("/scripts/13b_ProcessForGO/Samplewise/outdir/4.1_SCIvsshamnaive_blood_Markers_KeepSeparate_dropRbMt.txt")
sampleID <- "4.1_3separate"
my_ident1 <- "blood" 
...
seurat_markers.tab2$cluster <- factor(seurat_markers.tab2$cluster, levels = c("blood_naive_veh", "blood_sham_veh",
                                                                              "blood_SCI_veh", "blood_SCI_ivig",
                                                                              "cord_SCI_veh", "cord_SCI_ivig"))
...
```{r}
keep <- names(df$sample[grep(my_ident1, df$sample)])
