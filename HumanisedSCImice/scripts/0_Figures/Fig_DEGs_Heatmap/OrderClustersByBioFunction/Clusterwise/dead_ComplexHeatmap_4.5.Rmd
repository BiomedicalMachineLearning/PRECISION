...
```{r}
seurat_markers.tab <- read.delim("/scripts/13b_ProcessForGO/Clusterwise/outdir/4.5_vehVSivig_cord_Markers_dropRbMt.txt")
sampleID <- "4.5"

df <- readRDS("/scripts/11b_RunClusteringRes0.4/out_obj/allintegratedClustered_Seurat2.RDS")

outdir <- "/scripts/0_Figures/Fig_DEGs_Heatmap/OrderClustersByBioFunction/Clusterwise/outdir/"

clusterID <- "integrated_snn_res.0.4"

desiredClusterOrder <- c("cl0", "cl1", "cl3", "cl4", "cl5", "cl2", "cl8", "cl6", "cl7", "cl9", "cl10")

my_ident1 <- "cord_SCI_ivig" 
my_ident2 <- "cord_SCI_veh"
```
...
seurat_markers.tab2$direction <- factor(seurat_markers.tab2$direction, levels = c("down", "up"))
...
# re-order by cluster and then by direction
#seurat_markers.tab2 <- seurat_markers.tab2[order(seurat_markers.tab2[,"clusterID"], seurat_markers.tab2[,"direction"] ),]
