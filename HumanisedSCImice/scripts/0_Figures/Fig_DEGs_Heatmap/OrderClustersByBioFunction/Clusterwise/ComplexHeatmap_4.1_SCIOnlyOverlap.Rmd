...
```{r}
seurat_markers.tab <- read.delim("/scripts/13b_ProcessForGO/Clusterwise/outdir/4.1_SCIvsshamnaive_blood_ComparisonVenn_Markers_RemoveRbMt_SCIonly.txt", row.names = 1)
...
seurat_markers.tab$cluster <- seurat_markers.tab$cluster %>% gsub("SCI_only_", "", .)
...
```{r}
seurat_markers.tab2 <- seurat_markers.tab[,c("gene", "cluster", "direction", "clusterID")]
