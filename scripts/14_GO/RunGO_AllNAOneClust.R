...
allIDs = mapIds(org.Hs.eg.db, column = "ENTREZID", keytype = "SYMBOL", keys = DE$gene)

prep_IDs <- function(list_to_test) {
  geneList <- pull(DE %>% dplyr::filter(cluster == list_to_test), "gene")
  if(all(!(is.na(allIDs[geneList])))) { #if at least some of the genes are not NAs
...
all_entrez <- Filter(length, all_entrez) # remove empty ones
