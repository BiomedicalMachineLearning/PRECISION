...
```{r}
outdir <- "/scripts/0_Figures/Fig_ReplotThesisFig3.7D/Option_facetTraumaGenes/outdir/"
...
# filter 2: annotate any overlaps between SCI vs naive and sham vs naive (in the same direction) so we can facet them
...
# add in an extra column (trauma gene) so I can facet later
GOdata_up <- mutate(GOdata_up, trauma_gene = ifelse((Description %in% GOup_removeNv), "yes", "no"))
GOdata_down <- mutate(GOdata_down, trauma_gene = ifelse((Description %in% GOdown_removeNv), "yes", "no"))

#merge
GOdata_facet <- rbind(GOdata_up, GOdata_down)
```
...
MakeDotplots <- function(myGOdata, n, width, height, sampleID) {
  ...
  #table <- topN_allhits[,c(1,2,3,4,5,7,11, 12, 13, 14, 15)]
  table <- topN_allhits
  ...
  dotplot_grp_topN <- table %>% filter(Description %in% markers) %>% 
    ...
    facet_wrap(~trauma_gene, scales = "free_y", dir = "v")
...
MakeDotplots(GOdata_facet, 10, 7, 6, "facet")
```
