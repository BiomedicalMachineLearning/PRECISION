...
```{r}
outdir <- "/scripts/0_Figures/Fig_ReplotGORedBlue/Samplewise/NonTraumaGenes/outdir/"

sampleID <- "nontraumagenes_3mainvenns"

ck_filt_simple <- readRDS("/scripts/14_GO/Samplewise_AllGenes_RemoveRbMt/outdir_4.1_AdditionalVennOverlaps/outdir_nonTraumaGenes_3MainCircles/nontrauma_top100GO_FiltSimp.RDS")
```
...
ck_filt_simple@compareClusterResult$comparison <- factor(ck_filt_simple@compareClusterResult$comparison, levels = c("shamVnaive", "sciVnaive", "sciVsham"))
```
...
MakeDotplots <- function(n, width, height) {
  ...
  # set the order
  topN_allhits$Cluster <- factor(topN_allhits$Cluster, levels = c("shamVnaive_up", "sciVnaive_up", "sciVsham_up", "shamVnaive_down", "sciVnaive_down", "sciVsham_down"))
  myorder <- unique(topN_allhits$Description[order(topN_allhits$Cluster)])
  topN_allhits$Description <- factor(topN_allhits$Description, levels = myorder)
  ...
  table$direction <- gsub("^.*_", "", table$Cluster)
