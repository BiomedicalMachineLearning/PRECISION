...
outdir <- "/scripts/0_Figures/Fig_scRNAseq_allCells/Panels/Fig1D_markers/outdir/"

markers <- read.delim("/scripts/11c_Rerun_integrated_clusterMarkers/out_data/all_top10markers.txt")
```

```{r}
top5 <- markers %>% filter(p_val_adj <= 0.05) %>% group_by(cluster) %>% arrange(-(avg_log2FC)) %>% dplyr::slice(1:5, with_ties = FALSE)
```
...
```{r}
dir.create(file.path(outdir, "dotplot"), showWarnings = FALSE)

DotPlot(df, features = unique(markers$gene)) + RotatedAxis()
ggsave(filename = paste0(outdir, "/dotplot/", "top10markers_dotplot.pdf"), width = 20, height = 6)

DotPlot(df, features = unique(top5$gene)) + RotatedAxis()
ggsave(filename = paste0(outdir, "/dotplot/", "top5markers_dotplot.pdf"), width = 12, height = 6)

for (i in unique(markers$cluster)) {
  top10_markers <- markers %>% filter(cluster == i) %>% pull(gene)
  DotPlot(df, features = top10_markers)
  ggsave(filename = paste0(outdir, "/dotplot/", i, "_top10_dotplot.pdf"), width = 10, height = 4)
  
  top5_markers <- top5 %>% filter(cluster == i) %>% pull(gene)
  DotPlot(df, features = top5_markers)
  ggsave(filename = paste0(outdir, "/dotplot/", i, "_top5_dotplot.pdf"), width = 6, height = 4)
}
```
...
for (i in unique(markers$cluster)) {
  ...
  for (j in top10_markers) {
    FeaturePlot(df, features = j, order = TRUE)
    ggsave(filename = paste0(outdir, "/featureplot/", "cl", i, "_", j, ".pdf"), width = 4.3, height = 4)
  }
}
```
