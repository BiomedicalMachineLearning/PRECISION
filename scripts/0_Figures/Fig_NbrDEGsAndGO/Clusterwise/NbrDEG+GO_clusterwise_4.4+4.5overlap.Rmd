...
```{r}
sampleID <- "4.4+4.5overlap"
markers_all <-read.delim("/scripts/13b_ProcessForGO/Clusterwise/outdir/4.4v4.5_ComparisonVenn_Markers.txt")
markers_noRb <- read.delim("/scripts/13b_ProcessForGO/Clusterwise/outdir/4.4v4.5_ComparisonVenn_Markers_RemoveRbMt.txt")
  
GO_noRb <- read.delim("/scripts/14_GO/Clusterwise_AllGenes_RemoveRbMt/outdir_4.4vs4.5/cf_4.4_4.5_top100GO_FiltSimp.txt")
...
# manually add in the extra samples to the noRb one:
temp <- merge(nbr_markers_noRb_temp, nbr_markers_all_temp, all = TRUE)
temp[is.na(temp$noRb),"noRb"] <- 0
temp[is.na(temp$noRb),"Rb"] <- 0
