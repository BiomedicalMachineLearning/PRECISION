...
```{r}
sampleID <- "4.1_shamVnaive"
markers_all <- read.delim("/scripts/13b_ProcessForGO/Samplewise/outdir/4.1_SCIvsshamnaive_blood_Markers_KeepSeparate.txt")
markers_noRb <- read.delim("/scripts/13b_ProcessForGO/Samplewise/outdir/4.1_SCIvsshamnaive_blood_Markers_KeepSeparate_dropRbMt.txt")
GO_noRb <- read.delim("/scripts/14_GO/Samplewise_AllGenes_RemoveRbMt/outdir_4.1_3ComparisonsSeparate/clust_4.1_separate_top100GO_FiltSimp.txt")

outdir <- "/scripts/0_Figures/Fig_NbrDEGsAndGO/Samplewise/outdir/"
```

```{r}
# filter GO to sham vs naive only
markers_all <- markers_all %>% filter(analysisID == "shamVnaive")
markers_noRb <- markers_noRb %>% filter(analysisID == "shamVnaive")
GO_noRb <- GO_noRb %>% filter(Cluster == "shamVnaive_down" | Cluster == "shamVnaive_up")

# add in a direction column
markers_all$direction <- markers_all$cluster
markers_all$direction <- gsub("shamVnaive_", "", markers_all$direction)
markers_noRb$direction <- markers_noRb$cluster
markers_noRb$direction <- gsub("shamVnaive_", "", markers_noRb$direction)
GO_noRb$direction <- GO_noRb$Cluster
GO_noRb$direction <- gsub("shamVnaive_", "", GO_noRb$direction)
```
...
```{r}
nbr_markers_all <- as.data.frame(table(markers_all$direction))
colnames(nbr_markers_all) <- c("direction", "# of markers")
nbr_markers_noRb <- as.data.frame(table(markers_noRb$direction))
...
```{r}
NbrGo <- as.data.frame(table(GO_noRb %>% filter(p.adjust <= 0.05) %>% pull(direction)))
