...
```{r}
seurat_markers.tab <- read.delim("/scripts/13b_ProcessForGO/Samplewise/outdir/4.2_bloodVScord_veh_Markers_dropRbMt.txt")
sampleID <- "4.2_dropClusters"
...
outdir <- "/scripts/0_Figures/Fig_DEGs_Heatmap/OrderClustersByBioFunction/Samplewise/outdir/"

clusterID <- "integrated_snn_res.0.4"

desiredClusterOrder <- c("0", "1", "4", "2", "9", "10")
```

Only include clusters 0, 1, 2, 4, 9, 10 (these are the clusters that were analysed for cord)

```{r}
keep <- rownames(df[[]])[df$integrated_snn_res.0.4 == 0 | df$integrated_snn_res.0.4 == 1 | df$integrated_snn_res.0.4 == 2 | df$integrated_snn_res.0.4 == 4 | df$integrated_snn_res.0.4 == 9 | df$integrated_snn_res.0.4 == 10]
...
```{r}
seurat_markers.tab2 <- seurat_markers.tab[,c("cluster", "gene")]
...
ht_maxOutOutliers_fullscale <- Heatmap(countdata_v3,
        ...
        bottom_annotation = ann_clust,
        top_annotation = ann_time,
        # legend
         heatmap_legend_param = list(title = NULL,
                                     labels_gp = gpar(fontsize = 8),
                                     direction = "horizontal", nrow=1),
        # split up clusters (cols) and additional genes (rows)
        column_split = myorder$sample, column_title = NULL,
        column_gap = unit(0.5, "mm"),
        use_raster = TRUE, raster_quality = 1,
        width = unit(7, "cm") #based on current figure
        )

pdf(paste0(outdir, sampleID, "_DEGs_heatmap_allcells.pdf"), height = 12)
...
```{r}
keep <- names(which(df$sample == my_ident1 | df$sample == my_ident2))
