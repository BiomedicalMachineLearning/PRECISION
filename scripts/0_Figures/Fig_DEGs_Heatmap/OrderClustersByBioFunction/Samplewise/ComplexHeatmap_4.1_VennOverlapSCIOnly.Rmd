...
```{r}
seurat_markers.tab <- read.delim("/scripts/13b_ProcessForGO/Samplewise/outdir/4.1_SCIvsshamnaive_blood_ComparisonVenn_Markers_RemoveRbMt_SCIonly.txt")
...
seurat_markers.tab2$cluster <- gsub("SCI_only_", "", seurat_markers.tab2$cluster)
...
```{r}
keep <- rownames(df[[]])[grep("blood", df$sample)]
